...
1.  **NEVER Push Directly to `main`**:
    ...
    *   **No exceptions.** Even maintainer-requested changes (reverts, docs, `TODO.md` updates) must go through a PR.
...
2.  **NEVER Create Tags or Deploy Without Explicit Permission**:
    ...
    *   **NEVER add a release script's safety-override flag on your own initiative**
        (e.g., `--allow-tagged-commit`, `--confirm-skipped-jobs`), even when you've
...
- **Configuration**:
  ...
  - **Adding a KMP library dep to a shared module can silently break downstream Compose-MP consumers, and dev-mode PR CI won't catch it**: dev-mode PR CI skips the `build_android` / `build_desktop` / `build_ios_compose` / `build_server` jobs, so a change to a widely-consumed module (`:viewmodel`, `:experimental:viewmodel`, etc.) can be all-green on the PR and break `main` on push. In particular, adding a dep that pulls a *different* `androidx.lifecycle` (Google's `androidx.lifecycle:*` vs the project's JetBrains `org.jetbrains.androidx.lifecycle:*`) can **evict a transitively-provided lifecycle artifact** from a consumer — e.g. adding `kmp-observableviewmodel-core` to `:experimental:viewmodel` evicted `lifecycle-runtime-compose`, breaking `collectAsStateWithLifecycle` in `:experimental:compose-app` (which relied on it transitively). Mitigations: (1) before merging a shared-module dependency change, build the Compose-MP consumers locally — at minimum `./gradlew :compose-app:assembleDebug :compose-app:compileKotlinDesktop :compose-app:compileKotlinIosSimulatorArm64 :experimental:compose-app:compileKotlinDesktop :experimental:compose-app:compileKotlinIosSimulatorArm64`; (2) declare needed transitive deps **explicitly** in each consumer (the main `:compose-app` already declares `androidx.lifecycle.runtimeCompose` directly — match that). Example: bb-ovm1 / PR #1250 caught + fixed this pre-merge.
  - **Changing SKIE / cinterop configuration can reshape the Swift API surface, and dev-mode PR CI won't catch it**: SKIE interop features do more than their names suggest. In particular `FlowInterop` is **load-bearing** for the KMP-ObservableViewModel `.value` accessors — beyond exposing Flows as `AsyncSequence`, it provides the strong Swift typing for `StateFlow<T>.value`. Disabling it silently degrades `.value` to `Any?` and breaks **every** Swift accessor (`error: type 'Any' does not conform to '<State>'`), even though Kotlin/commonMain compiles fine. A static "is this interop used?" sweep misses this (no `AsyncSequence`/`await` call sites exist, yet the typing dependency is real). `SuspendInterop` *is* safely disabled (no Swift `await` on a Kotlin `suspend` fn). Before merging any `skie { }` / cinterop / `binaries.framework { export(...) }` change, link the framework and run `./scripts/build-ios.sh` locally — dev-mode PR CI skips `build_ios_native` / `validation_ios_ui`. Example: PR #1256 (disabling `FlowInterop` broke `ObservableViewModelBridge.swift`; pared back to `SuspendInterop`-only before merge).
  - **A squash-merge can silently trigger NO post-merge CI run — verify the run exists before reporting post-merge status.** Observed 2026-06-24: merging PR #1265 produced commit `1e7514ad` on `main`, but `gh api "repos/cartland/battery-butler/actions/runs?head_sha=<sha>"` returned zero runs — no `push`-event run was ever created (not a concurrency cancellation; push concurrency is SHA-based with `cancel-in-progress:false`). Do not assume "merged ⇒ post-merge CI ran." After merging something you need CI to verify (especially iOS, which dev-mode PR CI skips), confirm a run exists for the merge SHA; if none, force a full run with `gh workflow run "Battery Butler CI" --ref main -f ci_mode=release` (the ci-mode override added in bb-cidsp/#1267 — a manual run otherwise skips the iOS/slow jobs in dev mode). **And never call a CI fix done from parity reasoning alone:** #1260 and #1265 were both merged as `build_ios_native` fixes "validated" only against the passing `validation_ios_ui` job, and both still failed once the job actually ran — the real cause (`CONFIGURATION_BUILD_DIR=build/` breaking SwiftPM module resolution, fixed in #1270) only surfaced when the job executed. Reproduce the failure and run the job before claiming a CI fix works. Related bash gotcha: launching a build as `cmd > log 2>&1; echo "EXIT=$?"` makes the harness completion notification report the *compound* command's exit (the trailing `echo`, always 0), masking a real `** BUILD FAILED **` — grep the log for `BUILD SUCCEEDED`/`BUILD FAILED` instead of trusting the reported exit code.
...
- **Self Improvements**:
  ...
  - **Always** run `/dump-context` before ending a session where significant work was done. This captures tasks, decisions, and operational knowledge into `TODO.md` and docs.

- **Session Start**:
  - Read `TODO.md` (repo root) to see current tasks.
  ...
  - For team sessions, use Claude's TaskCreate/TaskList for coordination — not `TODO.md`.
...
- **Pull Requests**:
  ...
  - **A failed `validation_no_blocking_issues` does NOT auto-rerun when blocking issues close.** The gate snapshot-checks issue state once when it runs. If the gate completed FAILURE while issues were open, closing the issues afterward leaves the check stuck at FAILURE and the PR stays BLOCKED even though there are no open blockers. The fix is `gh run rerun <pr-run-id> --failed` to re-run only the failed jobs. Auto-merge enabled on the PR will then fire on the fresh-passing gate. Observed: PRs #1182 and #1183 both needed this rerun on 2026-05-12 after issues #1180/#1181 were manually closed.
  - **Transient CI infra failures are not code regressions — inspect before reverting.** Common today: `INSTALL_FAILED_VERIFICATION_FAILURE` (Android emulator install timeout in `validation_instrumented`) and run-level `cancelled` (push-to-main concurrency regression, see bb-w73e in `TODO.md`). Both file blocking ci-failure issues via the post-merge safety net. Before opening a revert PR: `gh run view <id> --log-failed | grep -E "FAIL|error|Exception"` → identify the failure type → `gh run rerun <id> --failed` for infra flakes, only revert on actual code-level failures (compile / test assertion / lint).
  - **Manual close of stale ci-failure blocking issues is acceptable** when (a) the underlying failing job has since passed on a rerun (`gh run view <id> --json jobs --jq '.jobs[] | select(.name=="<job>") | {name, conclusion}'`), AND (b) PR auto-merges are waiting on the stale block. Always reference the resolved-rerun's run ID and the actual fix path in the close comment. The post-merge safety net will file a fresh issue if the failure recurs — no danger of hiding a real break.
