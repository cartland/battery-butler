...
`risky` matches `gradle/libs.versions.toml`, `gradle/**`, `gradlew*`, `**/build.gradle.kts`, `settings.gradle.kts`, `buildSrc/**`, `.github/workflows/**`, `.github/actions/**`, plus `domain/**` and `presentation-model/**`.

**Why the last two are in there.** Kotlin/Native exports those types to Swift as a single designated initializer taking every parameter, and Kotlin default arguments do **not** cross the boundary — so adding a defaulted parameter to a state class or domain model is source-compatible for Kotlin and a compile error for every Swift call site. Swift constructs them directly (`TestData.swift` builds `Device`/`DeviceType`/`BatteryEvent`; the `*ScreenTests` build the screen states). Verified against the generated framework: the ObjC header declares exactly one initializer and SKIE's `DefaultArgumentInterop`, although enabled, generates no defaulted overload for these constructors. This broke twice in one day (2026-07-31) before the filter was widened.

A bespoke buildSrc parser that compared Kotlin constructor parameters against Swift call sites was written and then deleted the same day. It duplicated a check the Swift compiler already performs definitively, and it scanned only `presentation-model` — so a parameter added to `Device` would have sailed past it. **When a compiler already checks something, make it run at the right time rather than reimplementing it.**
...
- `com.google.cloud.tools:jib-gradle-plugin` — pinned to 3.4.1. Jib 3.5.x bytecode calls a `putArchiveEntry(TarArchiveEntry)` overload that commons-compress no longer provides. PR #1133 bumped past this and broke `build_server` on main; reverted in PR #1146 and added to the ignore list. See `build.gradle.kts` for the resolution-strategy forces and the upstream issue [jib#4235](https://github.com/GoogleContainerTools/jib/issues/4235). **Do not try to unpin this on the "commons-compress is new enough now" theory — it was tested on 2026-07-31 and is wrong.** jib 3.5.4 *calls* `putArchiveEntry(TarArchiveEntry)`, an overload commons-compress dropped, so `:server:app:jibBuildTar` dies with `NoSuchMethodError` on that exact signature against commons-compress **1.25.0, 1.27.1 and 1.28.0** alike — with all four sites (the `classpath()`, both `force()` blocks, and `libs.versions.toml`) changed together each time. The direction of the constraint recorded earlier was backwards: jib 3.5.x does not *require* newer commons-compress, it requires an overload newer versions removed. **jib 3.4.1 + commons-compress 1.28.0 is the combination that works.** Separately, a version-catalog-only bump is silently overridden by those forces, which is why PR #1426 could never have worked either.
...
- `io.nlopez.compose.rules:detekt` (>= 0.6.0) — 0.6.x bundles a config schema declaring every Compose rule with only `active:` and no per-rule properties, so `config.validation: true` rejects our `CompositionLocalAllowlist.allowedCompositionLocals` list and detekt fails in 28 modules (including ones with no Compose code). The rule still *reads* the property; only validation objects. `config.excludes` does not suppress it. Dependabot PR #1442 closed. 0.5.x is untested and deliberately left free — see TODO `bb-compose-rules-schema`.
...
- **`./scripts/validate.sh` can hang for an hour+ with zero output on `:server:app:jibBuildTar`** (observed 2026-07-06) — likely a stuck container-registry network call, unrelated to whether your change actually touches the server. If you piped its output through `tail -N` in the background, `tail` (without `-f`) buffers everything and shows nothing until the whole script exits, which can make a genuinely hung process look like it's merely slow with no visibility. Check for real progress with `ps aux | grep -i gradle` (a `jibBuildTar` gradlew invocation sitting for a long time with no CPU movement is the tell) before assuming it's just slow. For a change scoped to specific modules, prefer running the individual checks (`spotlessCheck`, `detekt`, `checkArchitecture`, `checkImportBoundary`, `checkNamingConventions`, `checkTestCoverage`, `checkThemeLayer`, `checkHardcodedStrings`, `checkDataStoreSingleton`, `checkPreviewTime`, plus targeted `:module:jvmTest`/`desktopTest`) directly rather than the full `validate.sh`, which also builds every app target (Android, Desktop, Server, Jib container, iOS) regardless of what your diff touches.
