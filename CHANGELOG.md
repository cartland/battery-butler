...
- **`build_ios_native` SPM-resolution fix** ([#1260](https://github.com/cartland/battery-butler/pull/1260), [#1265](https://github.com/cartland/battery-butler/pull/1265)): The `build_ios_native` job had been red on `main` since the migration (#1250) — it cached Xcode DerivedData and built without `clean`/package re-resolution, so a stale restored cache left xcodebuild unable to find the `KMP-ObservableViewModel` SPM modules. (The same app builds fine in `validation_ios_ui`, which uses no DerivedData cache + `clean build`.) #1260's `-resolvePackageDependencies` step was insufficient — packages resolved but stale build intermediates persisted — so #1265 dropped the DerivedData cache and switched to `clean build`, mirroring `validation_ios_ui`. Formal on-`main` re-confirmation is pending (the #1265 merge triggered no push CI run); see `bb-iosv` in `TODO.md`.
