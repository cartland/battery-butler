...
- **Local database isolation now keys off a NetworkMode's full identity, not just its subtype** ([#1324](https://github.com/cartland/battery-butler/pull/1324)): `DatabaseOption.fromNetworkMode()` previously matched only on the `NetworkMode` sealed subtype (e.g. `GrpcAws`), discarding the `url` payload that `GrpcLocal`/`GrpcAws`/`GrpcDev`/`LabsStaging`/`LabsProd` all carry. Two different real backends sharing a subtype but a different URL — reachable today via the `ServerUrlReceiver` adb debug tool — would silently share one SQLite file with no origin marker and last-write-wins-by-id inserts. `DatabaseOption` is now a data class keyed on `(category, fileName)`, where `fileName` incorporates a deterministic hash of the normalized URL when the mode carries one. A one-time file-rename migration (`promoteBareFileIfNeeded`) inherits an existing install's bare per-category file for whichever URL is currently configured, so this is backward compatible for the common case — only a genuinely different URL under the same category gets a new, isolated file. Prompted by a user request to review data isolation across network-mode switches; the `NetworkMode` → `DataLocation` rename floated during that review was deliberately deferred (`bb-data-location-rename` in `TODO.md`) to keep the safety fix small and reviewable on its own.
...
- **Fresh-install default backend prefers prod** ([#1303](https://github.com/cartland/battery-butler/pull/1303)): a fresh install now defaults to **Labs prod** when `LABS_PROD_URL` is configured, else Labs staging when configured, else offline (`None`) — so a release built with the prod host configured defaults real users to prod, not staging. Logic extracted to a pure, unit-tested `defaultNetworkMode(prodUrl, stagingUrl)`.
...
- **SKIE shrunk + bumped 0.10.9 → 0.10.12** ([#1256](https://github.com/cartland/battery-butler/pull/1256)): Bumped SKIE (now supports Kotlin up to 2.3.21 — so SKIE is no longer the gate for a Kotlin 2.3.x bump; that's now KMP-ObservableViewModel, see bb-k4sk) and added an explicit `skie { }` block to both SKIE modules disabling the unused `SuspendInterop` (no Swift code awaits a Kotlin `suspend` function). **`FlowInterop` is deliberately kept and documented as load-bearing**: beyond exposing Flows as `AsyncSequence`, it provides the strong Swift typing for `StateFlow.value` that the KMP-ObservableViewModel `.value` accessors depend on — disabling it degrades `.value` to `Any?` and breaks every accessor. Found via a local iOS build (dev-mode PR CI skips iOS).
...
- **Two silent SwiftUI non-update bugs + an observability guard** ([#1255](https://github.com/cartland/battery-butler/pull/1255)): `DeviceTypeListViewModel.actionError` and `AddDeviceTypeViewModel.actionError` exposed a *plain* `kotlinx` `MutableStateFlow`, which does not drive `@StateViewModel` re-renders — SwiftUI silently never updated on those errors (no compile error, and dev-mode PR CI skips iOS so it would only surface post-merge). Switched them to the observable factory (private *funnel* flows stay plain). Added `ExposedStateObservabilityConventionTest` (`:viewmodel:desktopTest`, runs on every PR via `./gradlew test`) that fails the build when exposed `(Mutable)StateFlow` state resolves to a plain `MutableStateFlow`. Also normalized `EditDeviceTypeScreen` to `collectAsStateWithLifecycle`.

### CI/CD

- **`build_ios_native` fixed — `CONFIGURATION_BUILD_DIR` broke SwiftPM** ([#1270](https://github.com/cartland/battery-butler/pull/1270); earlier mis-diagnoses [#1260](https://github.com/cartland/battery-butler/pull/1260)/[#1265](https://github.com/cartland/battery-butler/pull/1265)): `build_ios_native` had been red on `main` since the migration (#1250) with "Unable to find module dependency: 'KMPObservableViewModelCore'". The real cause was `CONFIGURATION_BUILD_DIR=build/` in the job's `xcodebuild` — overriding it makes SwiftPM package products land in a flat dir the app target's `swiftc` doesn't search (only `iosAppSwiftUI` consumes an SPM package, which is why the other iOS apps built fine *with* it). #1260 (`-resolvePackageDependencies`) and #1265 (drop DerivedData cache + `clean build`) mis-blamed a stale cache and did not fix it. #1270 drops `CONFIGURATION_BUILD_DIR` and builds into `-derivedDataPath build/ios-build`, matching the passing `validation_ios_ui` job and `scripts/build-ios.sh`. **Verified green on `main`** (push run for `f6e71369`). Also added a `workflow_dispatch` ci-mode override ([#1267](https://github.com/cartland/battery-butler/pull/1267)) so the full suite (incl. iOS jobs) can be run on demand — which is what made the diagnosis possible.
...
- **CI post-merge auto-issue safety net**: New `ci-post-merge-issue.yml` workflow listens for every `Battery Butler CI` run on a push to `main`. On failure it opens or comments on an issue per failing job (label `ci-failure` + `blocking`, deduped by exact title). On success it closes every open `ci-failure` issue. This is the safety net that makes development-mode CI viable as the steady state — regressions caught post-merge surface as tracked issues, not silent red. Paired with the `/check-ci-issues` slash command (lists open ci-failure issues; intended as a session-start ritual).
