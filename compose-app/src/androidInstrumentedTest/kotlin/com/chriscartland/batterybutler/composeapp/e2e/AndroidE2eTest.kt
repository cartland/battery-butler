...
class AndroidE2eTest {
    ...
    fun setUp() {
        runBlocking {
            ...
            val publicOkHttpClient = OkHttpClient
                .Builder()
            ...
            val publicGrpcClient = GrpcClient
                .Builder()
                .client(publicOkHttpClient)
                .baseUrl(serverUrl)
                .build()

            val authClient = com.chriscartland.batterybutler.proto
                .GrpcAuthServiceClient(publicGrpcClient)

            // 3. Login (Dev Mode)
            val sessionToken = try {
                val response = authClient.VerifyToken().execute(
                    com.chriscartland.batterybutler.proto
                        .VerifyTokenRequest(google_id_token = "dev-token"),
                )
                if (!response.valid) {
            ...
            val authOkHttpClient = publicOkHttpClient
                .newBuilder()
