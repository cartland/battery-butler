<!-- GENERATED FILE - DO NOT EDIT -->

# Code Analysis

This document provides a breakdown of the codebase by application layer and module.
Total Lines of Code: 41653

## Application Breakdown
* Shared Code: 26,882 lines (64.5%)
* iOS Swift App: 6,101 lines (14.6%)
* Other: 3,505 lines (8.4%)
* CMP Android, iOS, Desktop: 2,458 lines (5.9%)
* Server: 1,528 lines (3.7%)
* Android Screenshot Tests: 1,179 lines (2.8%)

## Module Breakdown
* `:iosAppSwiftUI.xcodeproj`: 6,101 lines (14.6%)
* `:presentation-feature`: 6,036 lines (14.5%)
* `:viewmodel`: 5,142 lines (12.3%)
* `:usecase`: 4,262 lines (10.2%)
* `:data-network`: 2,429 lines (5.8%)
* `:domain`: 2,284 lines (5.5%)
* `:presentation-core`: 2,281 lines (5.5%)
* `:compose-app`: 2,217 lines (5.3%)
* `:data`: 1,832 lines (4.4%)
* `:buildSrc`: 1,799 lines (4.3%)
* `:server:app`: 1,342 lines (3.2%)
* `:android-screenshot-tests`: 1,179 lines (2.8%)
* `:data-local`: 1,059 lines (2.5%)
* `:test-common`: 620 lines (1.5%)
* `:ai`: 493 lines (1.2%)
* `:experimental:data-local`: 332 lines (0.8%)
* `:e2e-tests`: 271 lines (0.7%)
* `:experimental:viewmodel`: 249 lines (0.6%)
* `:experimental:compose-app`: 241 lines (0.6%)
* `:presentation-model`: 218 lines (0.5%)
* `:fixtures`: 209 lines (0.5%)
* `:experimental:usecase`: 205 lines (0.5%)
* `:ios-swift-di`: 182 lines (0.4%)
* `:ExperimentalApp.xcodeproj`: 175 lines (0.4%)
* `:experimental:presentation-core`: 165 lines (0.4%)
* `:server:data`: 152 lines (0.4%)
* `:compose-resources`: 70 lines (0.2%)
* `:experimental:domain`: 43 lines (0.1%)
* `:server:domain`: 34 lines (0.1%)
* `:iosAppComposeUI.xcodeproj`: 31 lines (0.1%)

## Language Breakdown
* Kotlin (.kt): 35,346 lines (84.9%)
* Swift (.swift): 6,307 lines (15.1%)

## Code Distribution

<!-- GENERATED:BEGIN code_distribution.mmd -->
```mermaid
---
config:
  sankey:
    showValues: true
    width: 800
    height: 1000
    nodeAlignment: justify
    linkColor: gradient
---
%% GENERATED FILE - DO NOT EDIT
sankey-beta

Codebase,Shared Code,26882
Codebase,iOS Swift App,6101
Codebase,Other,3505
Codebase,CMP Apps,2458
Codebase,Server,1528
Codebase,Screenshot Tests,1179

Shared Code,presentation-feature,6036
Shared Code,viewmodel,5142
Shared Code,usecase,4262
Shared Code,data-network,2429
Shared Code,domain,2284
Shared Code,presentation-core,2281
Shared Code,data,1832
Shared Code,data-local,1059
Shared Code,ai,493
Shared Code,experimental:data-local,332
Shared Code,experimental:viewmodel,249
Shared Code,experimental:usecase,205
Shared Code,experimental:presentation-core,165
Shared Code,compose-resources,70
Shared Code,experimental:domain,43

iOS Swift App,iosAppSwiftUI,6101

Other,buildSrc,1799
Other,test-common,620
Other,e2e-tests,271
Other,presentation-model,218
Other,fixtures,209
Other,ios-swift-di,182
Other,ExperimentalApp,175
Other,iosAppComposeUI,31

CMP Apps,compose-app,2217
CMP Apps,experimental:compose-app,241

Server,server:app,1342
Server,server:data,152
Server,server:domain,34

Screenshot Tests,android-screenshot-tests,1179

iosAppSwiftUI,Swift,6101
presentation-feature,Kotlin,6036
viewmodel,Kotlin,5142
usecase,Kotlin,4262
data-network,Kotlin,2429
domain,Kotlin,2284
presentation-core,Kotlin,2281
compose-app,Kotlin,2217
data,Kotlin,1832
buildSrc,Kotlin,1799
server:app,Kotlin,1342
android-screenshot-tests,Kotlin,1179
data-local,Kotlin,1059
test-common,Kotlin,620
ai,Kotlin,493
experimental:data-local,Kotlin,332
e2e-tests,Kotlin,271
experimental:viewmodel,Kotlin,249
experimental:compose-app,Kotlin,241
presentation-model,Kotlin,218
fixtures,Kotlin,209
experimental:usecase,Kotlin,205
ios-swift-di,Kotlin,182
ExperimentalApp,Swift,175
experimental:presentation-core,Kotlin,165
server:data,Kotlin,152
compose-resources,Kotlin,70
experimental:domain,Kotlin,43
server:domain,Kotlin,34
iosAppComposeUI,Swift,31
```
<!-- GENERATED:END code_distribution.mmd -->

