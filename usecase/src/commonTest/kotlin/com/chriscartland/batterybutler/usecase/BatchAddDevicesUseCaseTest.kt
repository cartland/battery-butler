...
class FakeDeviceRepository : DeviceRepository {
    ...
    override suspend fun deleteDevice(id: String) { devices.removeIf { it.id == id } }
    ...
    override suspend fun addDeviceType(type: DeviceType) { deviceTypes.add(type) }
    override suspend fun updateDeviceType(type: DeviceType) {
        deviceTypes.removeIf { it.id == type.id }
        deviceTypes.add(type)
    }
    override suspend fun deleteDeviceType(id: String) { deviceTypes.removeIf { it.id == id } }
...
class FakeAiEngine : AiEngine {
    ...
    override suspend fun generateResponse(prompt: String, toolHandler: ToolHandler?): Flow<AiMessage> {
...
class BatchAddDevicesUseCaseTest {
    ...
    @Test
    fun invoke_adds_device_and_type_correctly() = runTest {
        ...
        assertEquals("Success: Added device 'Test Device' (Type: Test Type)", results.last().text)
    }
}
